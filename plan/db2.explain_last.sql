...
SELECT "Explain Plan"
  FROM (
SELECT LPAD(id,        MAX(LENGTH(id))        OVER(), ' ')
            || ' | '
            ...
            || CASE WHEN COUNT(ActualRows) OVER () > 1 -- the heading 'ActualRows' is always present, so "1" means no OTHER values
                    ...
                    ELSE ''
               END
            || LPAD(cost,      MAX(LENGTH(cost))      OVER(), ' ') "Explain Plan"
     , path
  FROM (
SELECT 'ID' ID
     , 'Operation' Operation
...
SELECT CAST(tree.operator_id as VARCHAR(254)) ID
     ...
     , COALESCE(CASE
         WHEN LTRIM(VARCHAR_FORMAT(O.Total_Cost, REPEAT('9', 40))) = '0' AND O.Total_Cost >= 1 THEN CAST(O.Total_Cost AS VARCHAR(40))
         ELSE LTRIM(VARCHAR_FORMAT(O.Total_Cost, REPEAT('9', 40)))
       END, '') Cost
     , path
  FROM tree
  LEFT JOIN ( SELECT i.source_id
              , i.target_id
              , CASE
                  WHEN LTRIM(VARCHAR_FORMAT(o.stream_count, REPEAT('9', 40))) = '0' AND o.stream_count >= 1 THEN CAST(o.stream_count AS VARCHAR(40))
                  ELSE LTRIM(VARCHAR_FORMAT(o.stream_count, REPEAT('9', 40)))
                END
                || ' of '
                || CASE
                  WHEN LTRIM(VARCHAR_FORMAT(total_rows, REPEAT('9', 40))) = '0' AND total_rows >= 1 THEN CAST(total_rows AS VARCHAR(40))
                  ELSE LTRIM(VARCHAR_FORMAT(total_rows, REPEAT('9', 40)))
                END
              ...
              , CASE WHEN act.actual_value is not null then
                CASE
                  WHEN LTRIM(VARCHAR_FORMAT(act.actual_value, REPEAT('9', 40))) = '0' AND act.actual_value >= 1 THEN CAST(act.actual_value AS VARCHAR(40))
                  ELSE LTRIM(VARCHAR_FORMAT(act.actual_value, REPEAT('9', 40)))
                END
                || ' of '
                || CASE
                  WHEN LTRIM(VARCHAR_FORMAT(total_rows, REPEAT('9', 40))) = '0' AND total_rows >= 1 THEN CAST(total_rows AS VARCHAR(40))
